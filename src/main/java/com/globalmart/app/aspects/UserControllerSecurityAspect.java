...
public class UserControllerSecurityAspect {

    private final UserSecurityService userSecurityService;

    @Before("execution(* com.globalmart.app.controllers.UserController.*(.., String)) && args(id,..)")
    public void checkUserAccess(JoinPoint joinPoint, String id) {
        String currentUsername = ((User) SecurityContextHolder.getContext()
                                                              .getAuthentication()
                                                              .getPrincipal()).getUsername();
        if (!userSecurityService.isAdmin(currentUsername) && !userSecurityService.isUserOwner(currentUsername, id)) {
